...
vitest_1.vi.mock("@/lib/db", () => {
    ...
    const mockStudentItemUpsert = vitest_1.vi.fn();
...
(0, vitest_1.describe)("P6 — Reward Atomicity", () => {
    ...
    (0, vitest_1.it)("succeeds on first attempt when transaction resolves", async () => {
        ...
        db_1.db.$transaction.mockImplementation(async (fn) => {
            ...
            const tx = {
                ...
                studentItem: { upsert: vitest_1.vi.fn().mockResolvedValue({}) },
                item: { findUnique: vitest_1.vi.fn().mockResolvedValue(makeItemStats()) },
    ...
    (0, vitest_1.it)("merges duplicate item drops into existing inventory rows", async () => {
        const gameStats = makeGameStats(5, 0, 200);
        const studentItemUpsert = vitest_1.vi.fn().mockResolvedValue({});
        db_1.db.$transaction.mockImplementation(async (fn) => {
            const tx = {
                student: {
                    findUnique: vitest_1.vi.fn().mockResolvedValue({ gameStats }),
        ...
        const results = await reward_manager_1.RewardManager.persistRewards([player]);
        (0, vitest_1.expect)(results[0].error).toBeUndefined();
        (0, vitest_1.expect)(studentItemUpsert).toHaveBeenCalledTimes(3);
        (0, vitest_1.expect)(studentItemUpsert).toHaveBeenNthCalledWith(1, vitest_1.expect.objectContaining({
            where: {
                studentId_itemId: {
                    studentId: "student-dup",
                    itemId: "item-1",
