...
export function LeaderboardTab({ classId, currentStudentId }: { classId: string; currentStudentId: string }) {
  ...
  const sorted = [...data].sort((a, b) => b.points - a.points).map((s, i) => ({ ...s, rank: i + 1 }));
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-cyan-50/40 to-white p-5 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">เรียงตามคะแนนพฤติกรรม</p>
        </div>
      ...
      {!loading && sorted.length >= 3 && (
        <div className="grid grid-cols-3 gap-4 px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            ...
            <p className="text-xs font-black text-indigo-600">⭐ {sorted[1].points.toLocaleString()} PTS</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <p className="text-xs font-black text-indigo-600">⭐ {sorted[0].points.toLocaleString()} PTS</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            ...
            <p className="text-xs font-black text-indigo-600">⭐ {sorted[2].points.toLocaleString()} PTS</p>
          </motion.div>
        </div>
      )}
            ...
            return (
              ...
              >
                <GlassCard className={`p-3.5 flex items-center gap-4 border-2 transition-all ${getRankBg(entry.rank, isMe)}`}>
                  <div className="w-8 flex items-center justify-center shrink-0">{getRankIcon(entry.rank)}</div>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`font-black text-sm leading-tight ${isMe ? "text-indigo-700" : "text-slate-800"}`}>
                      {entry.name}{" "}
                      {isMe && <span className="text-[10px] bg-indigo-500 text-white px-1.5 py-0.5 rounded-full ml-1">คุณ</span>}
                    </p>
                    <span className="text-[10px] font-bold text-indigo-500">⭐ {entry.points.toLocaleString()} พฤติกรรม</span>
                  </div>
