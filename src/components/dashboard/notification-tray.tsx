...
interface NotificationTrayProps {
    studentCode?: string // If provided, fetch from student API, else session API
}
...
export function NotificationTray({ studentCode }: NotificationTrayProps) {
    ...
    useEffect(() => {
        fetchNotifications()
        // Simple polling for real-time feel (though partial)
    ...
    const getIcon = (type: string) => {
        switch (type) {
            ...
            case "ASSIGNMENT": return <BookOpen className="w-4 h-4 text-indigo-500" />
            case "POINT": return <Star className="w-4 h-4 text-amber-500" />
            ...
            <PopoverContent className="w-80 md:w-96 p-0 bg-white/80 backdrop-blur-2xl border-white shadow-2xl rounded-[2rem] overflow-hidden" align="end">
                <div className="flex items-center justify-between px-6 py-4 bg-slate-50/50 border-b border-slate-100/50">
                    ...
                    {unreadCount > 0 && (
                        ...
                        >
                            <Check className="w-3 h-3 mr-1" /> ทั้งหมด
                        </Button>
                    )}
                </div>
                    ...
                    {loading ? (
                        <div className="p-8 text-center space-y-3">
                            <div className="flex justify-center flex-wrap gap-1">
                                {[1,2,3].map(i => <div key={i} className="h-1 w-1 bg-slate-200 rounded-full animate-bounce" style={{ animationDelay: `${i*0.2}s` }} />)}
                            </div>
                            <p className="text-xs text-slate-400 font-medium">กำลังโหลด...</p>
                        </div>
                        ...
                        <div className="p-12 text-center">
                            ...
                            <h4 className="font-bold text-slate-800 text-sm">ไม่มีการแจ้งเตือน</h4>
                            <p className="text-xs text-slate-400 mt-1">ข้อความใหม่จะแสดงที่นี่</p>
                        </div>
                    ) : (
                        <div className="py-2">
                                ...
                                {notifications.map((notif) => (
                                    <motion.div
                                        ...
                                        onClick={() => {
                                            ...
                                            if (notif.link) {
                                                setOpen(false)
                                                // Check for valid absolute/relative path
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-start justify-between gap-2">
                                                <h4 className={cn("text-sm leading-tight text-slate-800", !notif.isRead ? "font-bold" : "font-medium")}>
                                                    {notif.title}
                                                </h4>
                                                <span className="text-[10px] text-slate-400 font-medium shrink-0 whitespace-nowrap">
                                                    {formatDistanceToNow(new Date(notif.createdAt), { addSuffix: true, locale: th }).replace("ประมาณ ", "")}
                                                </span>
                                            </div>
                                            ...
                                            {notif.link && (
                                                <div className="flex items-center gap-1 mt-2 text-[10px] font-bold text-indigo-600">
                                                    ดูรายละเอียด <ExternalLink className="w-2.5 h-2.5" />
                                                </div>
                                            )}
                                        </div>
