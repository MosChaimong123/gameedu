...
const roundLogLines: string[] = [];
let lastNegamonRoundResult: NegamonRoundResultPayload | null = null;

const mockIo = {
    to: () => ({
        emit: (event: string, payload: NegamonRoundResultPayload) => {
            if (event === "negamon-round-result" && Array.isArray(payload?.logs)) {
                roundLogLines.push(...payload.logs);
                lastNegamonRoundResult = payload;
            }
        },
    }),
...
describe("NegamonBattleEngine", () => {
    beforeEach(() => {
        roundLogLines.length = 0;
        lastNegamonRoundResult = null;
    });
    ...
    it("normalizes per-hit damage and resolves strikes in answer-time order (sequential HP)", () => {
    ...
    it("skips eliminated targets for later strikers in the same round", () => {
        const engine = new NegamonBattleEngine("555555", "host", "set-1", {}, [baseQuestion], mockIo);
        engine.addPlayer(player("s1", "A"), sock("s1"));
        engine.addPlayer(player("s2", "B"), sock("s2"));
        engine.addPlayer(player("s3", "C"), sock("s3"));
        engine.startGame();
        ...
        engine.handleEvent(
            "submit-negamon-answer",
            { pin: "555555", questionId: "q1", answerIndex: 0 },
            sock("s1")
        );
        engine.handleEvent(
            "submit-negamon-answer",
        ...
        expect(b.battleHp).toBe(0);
        const hitsOnB = roundLogLines.filter((line) => line.includes("โจมตี B"));
        expect(hitsOnB.length).toBe(1);
        const payloadHitsOnB = (lastNegamonRoundResult?.hits ?? []).filter((h) => h.targetName === "B");
        expect(payloadHitsOnB.length).toBe(1);
        expect(payloadHitsOnB[0]?.attackerName).toBe("A");
    });
});
