...
async function main() {
    ...
    // Remove previous demo teacher + all related data (classrooms cascade to students, skills, assignments, …)
    ...
    if (existingTeacher) {
        const uid = existingTeacher.id;

        for (const c of existingTeacher.classrooms) {
            await prisma.classroom.delete({ where: { id: c.id } });
        }

        await prisma.gameHistory.deleteMany({ where: { hostId: uid } });
        await prisma.activeGame.deleteMany({ where: { hostId: uid } });
        ...
        await prisma.questionSet.deleteMany({ where: { creatorId: uid } });
        await prisma.folder.deleteMany({ where: { creatorId: uid } });
        await prisma.oMRQuiz.deleteMany({ where: { teacherId: uid } });

        await prisma.user.delete({ where: { id: uid } });
        console.log("🧹 Removed previous teacher@test.com demo data");
    }
