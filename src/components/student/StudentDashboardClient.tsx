            ...
            <div className="max-w-6xl mx-auto px-4 py-8 relative z-10">
                ...
                <div className="grid md:grid-cols-4 gap-8">
                    ...
                    <div className="md:col-span-3 space-y-8">
                        ...
                        >
                            <div className="w-full overflow-x-auto scrollbar-hide -mx-4 px-4 sm:mx-0 sm:px-0 mb-6 pb-2">
                                <TabsList className="bg-white/40 backdrop-blur-md p-1.5 rounded-2xl border border-white/50 shadow-sm w-max sm:w-auto flex">
                                    ...
                                    <TabsTrigger value="shop" className="rounded-xl px-8 py-2.5 flex items-center gap-2.5 data-[state=active]:bg-white data-[state=active]:text-amber-600 data-[state=active]:shadow-md font-black text-slate-500 transition-all">
                                        <ShoppingBag className="w-4 h-4" />
                                        <span>ร้านค้า</span>
                                    </TabsTrigger>
                                    <TabsTrigger value="inventory" className="rounded-xl px-5 py-2.5 flex items-center gap-2.5 data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-md font-black text-slate-500 transition-all">
                                        <Package className="w-4 h-4" />
                                        <span>คลังแสง</span>
                                    </TabsTrigger>
                                    <TabsTrigger value="achievements" className="rounded-xl px-5 py-2.5 flex items-center gap-2.5 data-[state=active]:bg-white data-[state=active]:text-amber-600 data-[state=active]:shadow-md font-black text-slate-500 transition-all">
                                        <Award className="w-4 h-4" />
                                        <span>ความสำเร็จ</span>
                                    </TabsTrigger>
                                    <TabsTrigger value="leaderboard" className="rounded-xl px-5 py-2.5 flex items-center gap-2.5 data-[state=active]:bg-white data-[state=active]:text-emerald-600 data-[state=active]:shadow-md font-black text-slate-500 transition-all">
                                        <BarChart3 className="w-4 h-4" />
                                        <span>อันดับ</span>
                                    </TabsTrigger>
                                    <TabsTrigger value="pvp" className="rounded-xl px-5 py-2.5 flex items-center gap-2.5 data-[state=active]:bg-white data-[state=active]:text-rose-600 data-[state=active]:shadow-md font-black text-slate-500 transition-all">
                            ...
                            <TabsContent value="assignments" className="mt-0 border-none p-0 outline-hidden space-y-8">
                                {/* Assignments Grid */}
                                <div>
                                    <h3 className="text-lg font-black text-slate-800 mb-4 px-2 flex items-center gap-2">
                                        <LayoutDashboard className="w-5 h-5 text-indigo-500" />
                                        ภารกิจที่ได้รับหมอบหมาย
                                    </h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {classroom.assignments?.filter((a: any) => a.visible !== false).map((assignment: any) => {
                                            ...
                                            const isCompleted = submission && score >= (assignment.passScore || maxScore * 0.5);
                                ...
                                <div className="pt-4">
                                    ...
                                    <div className="bg-white/40 backdrop-blur-md rounded-[2rem] border border-white/60 divide-y divide-slate-100 overflow-hidden">
                                        {initialHistory.length === 0 ? (
                                            <div className="p-12 text-center text-slate-400 font-bold">ยังไม่มีประวัติการได้รับคะแนน</div>
                                        ) : (
                                            initialHistory.map((h: any, idx: number) => (
                                                <div key={idx} className="p-4 flex items-center justify-between hover:bg-white/40 transition-colors">
                                                    <div className="flex items-center gap-4">
                                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${h.value > 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
                                                            <Award className="w-5 h-5" />
                                                        </div>
                                                        <div>
                                                            <p className="font-black text-slate-800 text-sm">{h.reason}</p>
                                                            <div className="flex items-center gap-2 text-[10px] text-slate-400 font-medium">
                                                                <Calendar className="w-3 h-3" />
                                                                {format(new Date(h.timestamp), 'd MMM yyyy HH:mm', { locale: th })}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <span className={`text-lg font-black ${h.value > 0 ? 'text-green-600' : 'text-red-600'}`}>
                            ...
                            <TabsContent value="leaderboard" className="mt-0 border-none p-0 outline-hidden">
                                <LeaderboardTab classId={student.classId} currentStudentId={student.id} />
                            </TabsContent>
