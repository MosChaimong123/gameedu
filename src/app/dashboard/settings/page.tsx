                ...
                <div className="flex flex-col gap-6 border-b border-border pb-10 md:flex-row md:items-end md:justify-between">
                    <div className="space-y-2">
                        <motion.div
                            ...
                            className="mb-4 flex h-16 w-16 items-center justify-center rounded-3xl bg-brand-pink text-white shadow-xl shadow-brand-pink/20"
                        >
                ...
                <Tabs defaultValue="language" className="w-full space-y-10">
                    <div className="flex justify-center">
                        <TabsList className="h-16 gap-2 rounded-3xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-200/50">
                            <TabsTrigger value="language" className="gap-2 rounded-2xl px-8 font-black transition-all data-[state=active]:bg-brand-pink data-[state=active]:text-white">
                                <Languages className="h-4 w-4" /> {t("userSettingsLanguageTab")}
                            </TabsTrigger>
                            <TabsTrigger value="audio" className="gap-2 rounded-2xl px-8 font-black transition-all data-[state=active]:bg-brand-pink data-[state=active]:text-white">
                                        ...
                                        <motion.button
                                            ...
                                            className={`group relative overflow-hidden rounded-[2.5rem] border-4 p-8 text-left transition-all ${
                                                settings.language === lang.id
                                                    ? "border-brand-pink bg-brand-pink/10 shadow-xl shadow-brand-pink/15"
                                                    : "border-slate-100 bg-slate-50 hover:border-brand-cyan/35"
                                            }`}
                                        >
                                            <div className="relative z-10 flex flex-col gap-4">
                                                <span className="block origin-left text-5xl transition-transform group-hover:scale-110">{lang.flag}</span>
                                                <div>
                                                    <h4 className={`text-2xl font-black leading-none ${settings.language === lang.id ? "text-brand-pink" : "text-foreground"}`}>
                                                        {t(lang.nameKey)}
                                                    </h4>
                                            ...
                                            {settings.language === lang.id && (
                                                <motion.div
                                                    layoutId="activeLang"
                                                    className="absolute right-6 top-6 flex h-10 w-10 items-center justify-center rounded-2xl bg-brand-pink text-white shadow-lg"
                                                >
                                                    <AnimatePresence>
                            ...
                            <CardContent className="space-y-8 p-10">
                                <div className="group flex items-center justify-between rounded-[2.5rem] border border-border bg-muted p-8 transition-all hover:bg-muted/50">
                                    <div className="flex items-center gap-6">
                                        <div className="rounded-3xl bg-brand-pink p-4 text-white shadow-lg shadow-brand-pink/20 transition-transform group-hover:rotate-6">
                                    ...
                                    <Switch
                                        ...
                                        className="scale-125 data-[state=checked]:bg-brand-pink"
                                    />
                                </div>
                ...
                {session?.user?.role === "ADMIN" && (
                    <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} className="pt-10">
                        <div className="group relative">
                            <div className="absolute -inset-1 rounded-[3.5rem] bg-brand-pink/25 blur" />
                            <Card className="relative overflow-hidden rounded-[3rem] border-0 bg-brand-pink p-2 text-white shadow-2xl">
                                <div className="space-y-8 p-10">
                                    <div className="flex items-center gap-6">
                                        <div className="rounded-[2rem] bg-white p-5 text-brand-pink shadow-2xl">
                                            <Shield className="h-10 w-10" />
                                        </div>
                                        <div className="space-y-1">
                                            <h2 className="text-4xl font-black italic leading-none">{t("userSettingsAdminCardTitle")}</h2>
                                            <p className="text-lg font-bold text-white/80">{t("userSettingsAdminCardSubtitle")}</p>
                                        </div>
                                    </div>
