...
(0, vitest_1.describe)("P8 — Job Multiplier Positivity", () => {
    (0, vitest_1.it)("all stats remain strictly positive after job multipliers", () => {
        ...
        fc.constantFrom(...JOB_CLASSES), fc.constantFrom(...JOB_TIERS), (points, level, jobClass, jobTier) => {
            ...
            (0, vitest_1.expect)(stats.luck).toBeGreaterThan(0);
        }), { numRuns: 300 });
    });
});
(0, vitest_1.describe)("Job passives + LUK scaling", () => {
    (0, vitest_1.it)("WARRIOR passives raise stats vs NOVICE at same level (empty equip)", () => {
        const level = 15;
        const novice = stat_calculator_1.StatCalculator.compute(0, [], level, null, "BASE");
        const warrior = stat_calculator_1.StatCalculator.compute(0, [], level, "WARRIOR", "BASE");
        const warriorMultipliers = (0, job_system_1.getStatMultipliers)("WARRIOR", "BASE");
        ...
        (0, vitest_1.expect)(warrior.hp).toBeGreaterThan(novice.hp);
        (0, vitest_1.expect)(warrior.def).toBeGreaterThan(warriorBaseDef);
        (0, vitest_1.expect)(warrior.atk).toBeGreaterThan(warriorBaseAtk);
    });
    (0, vitest_1.it)("RANGER scales LUK by job crit multiplier like IdleEngine", () => {
        const level = 10;
        const ranger = stat_calculator_1.StatCalculator.compute(0, [], level, "RANGER", "BASE");
        const baseLuck = 0.01 + level * 0.001;
        const expected = Number((baseLuck * 1.3).toFixed(3));
        (0, vitest_1.expect)(Math.abs(ranger.luck - expected)).toBeLessThan(0.002);
    });
});
