...
async function GET(req, { params }) {
    ...
    try {
        ...
        let totalItems = 0;
        const goldPerStudent = [];
        const achievementCounts = {};
        const attendanceSummary = { PRESENT: 0, ABSENT: 0, LATE: 0, LEFT_EARLY: 0 };
        for (const student of classroom.students) {
            ...
            for (const record of student.history) {
                ...
                if (recDate in dailyGrowth)
                    dailyGrowth[recDate] += Math.abs(record.value);
                recentHistory.push({
                    id: record.id,
        ...
        const studentStats = classroom.students.map((s) => {
            ...
            return {
                id: s.id,
                name: s.name,
                nickname: (_a = s.nickname) !== null && _a !== void 0 ? _a : null,
                points: s.points,
                gold: ((_b = s.gameStats) === null || _b === void 0 ? void 0 : _b.gold) || 0,
                totalPositive: pos,
                totalNeedsWork: neg,
                attendance: att,
