...
export type EnhancementZone = "SAFE" | "RISK" | "DANGER";

export interface EnhancementCost {
  gold: number;
  behaviorPoints: number;
  materialType: string | null;  // null for SAFE zone
  materialQuantity: number;     // 0 for SAFE/RISK zones
}

export interface EnhancementResult {
  success: boolean;
  newLevel: number;
  zone: EnhancementZone;
}

/** Tier max enhancement levels */
export const TIER_MAX: Record<string, number> = {
  COMMON:    9,
  RARE:      12,
  EPIC:      15,
  LEGENDARY: 15,
 ...
 */
export function getEnhancementZone(currentLevel: number): EnhancementZone {
  if (currentLevel <= 5)  return "SAFE";
  if (currentLevel <= 10) return "RISK";
  return "DANGER";
}
 ...
 */
export function rollEnhancement(currentLevel: number): EnhancementResult {
  const zone = getEnhancementZone(currentLevel);
