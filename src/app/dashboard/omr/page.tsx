...
export default function OMRDashboardPage() {
    ...
    const [deleteTarget, setDeleteTarget] = useState<OMRQuiz | null>(null)
    ...
    const fetchQuizzes = useCallback(async () => {
        try {
            ...
            if (!res.ok) {
                throw new Error(
                    await getLocalizedOmrErrorMessageFromResponse(res, "omrLoadQuizzesError", t, language)
                )
            }
            ...
            const description =
                net ?? (error instanceof Error ? error.message : t("omrLoadQuizzesError"))
    ...
    const handleCreateQuiz = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(
                    await getLocalizedOmrErrorMessageFromResponse(res, "omrCreateQuizError", t, language)
                )
            }
            const data = await res.json()
            ...
            toast({
                title: t("omrToastQuizCreatedTitle"),
                description: t("omrToastQuizCreatedDesc"),
            })
        } catch (error) {
            const raw = error instanceof Error ? error.message : null
            const net = tryLocalizeFetchNetworkFailureMessage(raw, t)
            const description = net ?? (error instanceof Error ? error.message : t("omrCreateQuizError"))
            toast({ title: description, variant: "destructive" })
        }
    }
    ...
    const handleUpdateKey = async (newKey: Record<string, string>) => {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(
                    await getLocalizedOmrErrorMessageFromResponse(res, "omrUpdateKeyError", t, language)
                )
            }
            const data = await res.json()
            ...
            toast({
                title: t("omrToastKeySavedTitle"),
                description: t("omrToastKeySavedDesc"),
            })
        } catch (error) {
            const raw = error instanceof Error ? error.message : null
            const net = tryLocalizeFetchNetworkFailureMessage(raw, t)
            const description = net ?? (error instanceof Error ? error.message : t("omrUpdateKeyError"))
            toast({ title: description, variant: "destructive" })
        }
    }
    ...
    const handleDeleteQuiz = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/omr/quizzes/${id}`, { method: "DELETE" })
            if (!res.ok) {
                throw new Error(
                    await getLocalizedOmrErrorMessageFromResponse(res, "omrDeleteQuizError", t, language)
                )
            }
            setQuizzes(quizzes.filter((q) => q.id !== id))
            setDeleteTarget(null)
            toast({
                title: t("omrToastQuizDeletedTitle"),
                description: t("omrToastQuizDeletedDesc"),
            })
        } catch (error) {
            const raw = error instanceof Error ? error.message : null
            const net = tryLocalizeFetchNetworkFailureMessage(raw, t)
            const description = net ?? (error instanceof Error ? error.message : t("omrDeleteQuizError"))
            toast({ title: description, variant: "destructive" })
        } finally {
    ...
    return (
        <div className="min-h-0 w-full">
            <div className="mx-auto w-full max-w-6xl">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                    <div className="space-y-4">
                        ...
                        <div>
                            <h1 className="text-4xl font-black text-slate-800 tracking-tight mb-2">{t("omrPageTitle")}</h1>
                            <p className="text-slate-500 font-bold uppercase tracking-widest text-xs">{t("omrPageSubtitle")}</p>
                        </div>
                    </div>
                    <Button 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="flex items-center justify-between px-4">
                            <h2 className="text-xl font-black text-slate-400 uppercase tracking-widest">{t("omrPageListTitle")}</h2>
                            <span className="bg-slate-200 px-3 py-1 rounded-full text-[10px] font-black text-slate-500">
                                {t("omrPageCountBadge", { count: quizzes.length })}
                            </span>
                        </div>
                        ...
                        {isLoading ? (
                            <div className="h-64 flex flex-col items-center justify-center bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
                                ...
                                <p className="font-black text-slate-400">{t("omrPageLoading")}</p>
                            </div>
                        ) : quizzes.length === 0 ? (
                            <div className="h-64 flex flex-col items-center justify-center bg-white rounded-[3rem] border-2 border-dashed border-slate-200 p-12 text-center">
                                ...
                                <h3 className="text-xl font-black text-slate-400 mb-2">{t("omrPageEmptyTitle")}</h3>
                                <p className="text-sm text-slate-400 font-medium max-w-xs">{t("omrPageEmptyDesc")}</p>
                            </div>
                        ) : (
                        ...
                        <div className="bg-slate-900 rounded-[3rem] p-8 text-white relative overflow-hidden shadow-2xl">
                            ...
                            <div className="space-y-4 relative z-10">
                                ...
                                <div className="p-4 bg-emerald-500/20 rounded-2xl border border-emerald-500/30 text-emerald-100 text-xs font-medium leading-relaxed">
                                    <p className="font-black text-emerald-400 mb-1 uppercase tracking-widest">{t("omrPageTipTitle")}</p>
                                    {t("omrPageTipBody")}
                                </div>
                            </div>
                        </div>
            ...
            <AnimatePresence>
                {isCreateModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                        ...
                        >
                            <h2 className="text-2xl font-black text-slate-800 mb-6 text-center">{t("omrWizardTitle")}</h2>
                            <div className="space-y-6">
                                <div>
                                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">{t("omrLabelExamName")}</label>
                                    <input 
                                        type="text" 
                                ...
                                <div>
                                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">{t("omrLabelNumQuestions")}</label>
                                    <div className="grid grid-cols-3 gap-3 mt-2">
                                        {[20, 50, 80].map((num) => (
                                            ...
                                            >
                                                {t("omrNumQuestionsOption", { count: num })}
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                <div className="flex gap-4 pt-4">
                                    <Button variant="ghost" className="flex-1 h-16 rounded-2xl font-black text-slate-400" onClick={() => setIsCreateModalOpen(false)}>{t("omrWizardCancel")}</Button>
                                    <Button className="flex-1 h-16 rounded-2xl bg-slate-900 text-white font-black" onClick={handleCreateQuiz}>{t("omrWizardCreate")}</Button>
                                </div>
                            </div>
                        </motion.div>
                    </div>
            ...
            <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>{t("omrDeleteTitle")}</AlertDialogTitle>
                        <AlertDialogDescription>
                            {deleteTarget
                                ? t("omrDeleteDescNamed", { title: deleteTarget.title })
                                : t("omrDeleteDescGeneric")}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel disabled={isDeletingQuiz}>{t("omrWizardCancel")}</AlertDialogCancel>
                        <AlertDialogAction
                            onClick={(event) => {
                                event.preventDefault()
                                if (!deleteTarget) return
                                void handleDeleteQuiz(deleteTarget.id)
                            }}
                            disabled={isDeletingQuiz}
                            className="bg-red-600 hover:bg-red-700"
                        >
                            {isDeletingQuiz ? t("omrDeleting") : t("omrDeleteAction")}
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
                        ...
                        >
                            <div className="flex items-center justify-between mb-8">
                                <div>
                                    <h2 className="text-3xl font-black text-slate-800 tracking-tight">{t("omrKeyManagerTitle")}</h2>
                                    <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px] mt-1">
                                        {t("omrQuizMeta", { title: selectedQuiz.title })}
                                    </p>
                                </div>
                                <div className="flex items-center gap-3">
                            ...
                            <div className="flex gap-4 mb-6 bg-slate-100 p-2 rounded-2xl w-fit">
                                ...
                                <Button 
                                    ...
                                    className="rounded-xl font-black"
                                >
                                    {t("omrViewResults", { count: selectedQuiz.results?.length || 0 })}
                                </Button>
                            </div>
                                ...
                                {selectedQuiz.showResults ? (
                                    <div className="space-y-4">
                                        {selectedQuiz.results?.length === 0 ? (
                                            <div className="text-center py-20 text-slate-400 font-bold italic">{t("omrNoScanRows")}</div>
                                        ) : (
                                            <table className="w-full text-left">
                                                <thead className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-200">
                                                    <tr>
                                                        <th className="pb-4">{t("omrColStudent")}</th>
                                                        <th className="pb-4 text-center">{t("omrColScore")}</th>
                                                        <th className="pb-4 text-right">{t("omrColActions")}</th>
                                                    </tr>
                                                </thead>
