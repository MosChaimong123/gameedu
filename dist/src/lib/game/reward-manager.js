        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            ...
            try {
                const result = await db_1.db.$transaction(async (tx) => {
                    ...
                    const currentStats = (0, game_stats_1.parseGameStats)(student.gameStats);
                    // 2. Apply time-based economy boosts (Lucky Scroll / Tome of Knowledge)
                    const now = Date.now();
                    const goldBoostActive = ((_a = currentStats.goldBoostExpiry) !== null && _a !== void 0 ? _a : 0) > now;
                    const xpBoostActive = ((_b = currentStats.xpBoostExpiry) !== null && _b !== void 0 ? _b : 0) > now;
                    ...
                    const { level: newLevel, xp: newXp, leveledUp } = idle_engine_1.IdleEngine.calculateXpGain(currentStats, finalXp);
                    // 4. Update gameStats: gold, xp, level
                    const updatedStats = {
                        ...currentStats,
                        gold: ((_c = currentStats.gold) !== null && _c !== void 0 ? _c : 0) + finalGold,
                        xp: newXp,
                        level: newLevel,
                    };
