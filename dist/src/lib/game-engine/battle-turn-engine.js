...
exports.computeBossDamageAgainstPlayer = computeBossDamageAgainstPlayer;
exports.computeDarkPactDrain = computeDarkPactDrain;
...
const game_stats_1 = require("../game/game-stats");
const skill_tree_1 = require("../game/skill-tree");
// ─── Boss Config ──────────────────────────────────────────────────────────────
const BOSS_CONFIG = {
    id: "co-op-boss-01",
...
function computeBossDamageAgainstPlayer(params) {
    const defenseMultiplier = params.hasTitanWill && params.playerHp / Math.max(1, params.playerMaxHp) < 0.3 ? 1.5 : 1;
...
function computeDarkPactDrain(maxHp) {
...
class BattleTurnEngine extends abstract_game_1.AbstractGameEngine {
    ...
    applyOutgoingDamageEffects(player, baseDamage, opts = {}) {
    ...
    tickStatusEffects(target, onDamage, onHeal) {
        target.statusEffects = target.statusEffects.filter((effect) => {
            var _a, _b;
            if (effect.type === "POISON")
                onDamage((_a = effect.value) !== null && _a !== void 0 ? _a : 0, "Poison");
            if (effect.type === "REGEN")
                onHeal((_b = effect.value) !== null && _b !== void 0 ? _b : 0, "Regen");
            effect.turnsRemaining--;
            return effect.turnsRemaining > 0;
        });
    }
    getIncomingDamageMultiplier(effects) {
        let mult = 1.0;
        for (const e of effects) {
            if (e.type === "ARMOR_PIERCE")
                mult += 0.20;
            if (e.type === "DEF_BREAK")
                mult += 0.50;
        }
    ...
    tickFarmingStatusEffects(player) {
        if (!player.soloMonster)
            return;
        // Monster DoT (POISON)
        this.tickStatusEffects(player.soloMonster, (dmg) => {
    ...
    addPlayer(player, socket) {
        ...
        const battlePlayer = {
            ...
            skills: [],
            isDefending: false,
            jobClass: (_c = player.jobClass) !== null && _c !== void 0 ? _c : null,
            jobTier: (_d = player.jobTier) !== null && _d !== void 0 ? _d : "BASE",
            wave: 1,
            soloMonster: null,
            studentId: (_e = player.studentId) !== null && _e !== void 0 ? _e : "",
            immortalUsed: false,
            hasLifesteal: false,
            hasImmortal: false,
            hasManaFlow: false,
    ...
    async loadPlayerStats(player) {
        ...
        try {
            ...
            const stats = stat_calculator_1.StatCalculator.compute((_b = student.points) !== null && _b !== void 0 ? _b : 0, (_c = student.items) !== null && _c !== void 0 ? _c : [], level, (_d = student.jobClass) !== null && _d !== void 0 ? _d : null, (_e = student.jobTier) !== null && _e !== void 0 ? _e : "BASE", (_f = student.advanceClass) !== null && _f !== void 0 ? _f : null);
            // ── Consumable buffs: apply pending HP bonus and Phoenix Feather ──────────
            const pendingHpBonus = (_g = gameStats.pendingHpBonus) !== null && _g !== void 0 ? _g : 0;
            const phoenixCharges = (_h = gameStats.phoenixCharges) !== null && _h !== void 0 ? _h : 0;
            const pendingBattleBuff = (_j = gameStats.pendingBattleBuff) !== null && _j !== void 0 ? _j : { atk: 0, def: 0, spd: 0 };
            const hasPotionBuff = pendingHpBonus > 0;
            const hasPhoenixBuff = phoenixCharges > 0;
            const hasBattleBuff = ((_k = pendingBattleBuff.atk) !== null && _k !== void 0 ? _k : 0) > 0 || ((_l = pendingBattleBuff.def) !== null && _l !== void 0 ? _l : 0) > 0 || ((_m = pendingBattleBuff.spd) !== null && _m !== void 0 ? _m : 0) > 0;
            if (hasPotionBuff || hasPhoenixBuff || hasBattleBuff) {
                ...
                await db_1.db.student.update({
                    where: { id: player.studentId },
                    data: { gameStats: (0, game_stats_1.toPrismaJson)(cleanedStats) },
                });
            }
            ...
            player.atk = hasBattleBuff ? Math.floor(stats.atk * (1 + ((_o = pendingBattleBuff.atk) !== null && _o !== void 0 ? _o : 0))) : stats.atk;
            player.def = hasBattleBuff ? Math.floor(stats.def * (1 + ((_p = pendingBattleBuff.def) !== null && _p !== void 0 ? _p : 0))) : stats.def;
            player.spd = hasBattleBuff ? Math.floor(stats.spd * (1 + ((_q = pendingBattleBuff.spd) !== null && _q !== void 0 ? _q : 0))) : stats.spd;
            player.crit = stats.crit;
            player.luck = stats.luck;
            player.mag = stats.mag;
            player.level = level;
            player.jobClass = (_r = student.jobClass) !== null && _r !== void 0 ? _r : null;
            player.jobTier = (_s = student.jobTier) !== null && _s !== void 0 ? _s : "BASE";
            player.skillTreeProgress = (_t = gameStats.skillTreeProgress) !== null && _t !== void 0 ? _t : {};
            ...
            player.dodgeChance = (_u = stats.dodgeChance) !== null && _u !== void 0 ? _u : 0;
            player.shadowVeilCritBuff = false;
            player.goldMultiplier = (_v = stats.goldMultiplier) !== null && _v !== void 0 ? _v : 0;
            player.xpMultiplier = (_w = stats.xpMultiplier) !== null && _w !== void 0 ? _w : 0;
            player.bossDamageMultiplier = (_x = stats.bossDamageMultiplier) !== null && _x !== void 0 ? _x : 0;
            ...
            if (Array.isArray(rawSkills)) {
                ...
                const classDef = (0, job_system_1.getMergedClassDef)(eff);
    ...
    executeBossAttackTick() {
        ...
        this.tickStatusEffects(this.boss, (dmg) => {
            if (!this.boss)
                return;
    ...
    applyDamageToBoss(rawDamage, attackerId, critApplied = false) {
        if (!this.boss)
            return;
        ...
        if (critApplied && (attacker === null || attacker === void 0 ? void 0 : attacker.chainLightningOnCrit) && this.boss && this.boss.hp > 0) {
        ...
        if (this.boss && this.boss.hp > 0 && (attacker === null || attacker === void 0 ? void 0 : attacker.hasEchoStrike) && Math.random() < 0.30) {
    ...
    executeSkillOnBoss(player, skillId, socket) {
        ...
        const baseSkill = (0, job_system_1.buildGlobalSkillMap)()[skillId];
        const rank = (0, skill_tree_1.getSkillRank)((_a = player.skillTreeProgress) !== null && _a !== void 0 ? _a : {}, skillId);
        ...
        this.emitBattleEvent({
            type: "ACTION_SKILL_CAST",
            sourceId: player.id,
            targetId: (_b = this.boss) === null || _b === void 0 ? void 0 : _b.id,
            skillId,
            label: skill.name,
            tone: "skill",
        });
        const isMagic = skill.damageBase === "MAG";
        const mult = (_c = skill.damageMultiplier) !== null && _c !== void 0 ? _c : (1 + skill.cost / 20);
        const effectiveAtk = this.getEffectivePlayerAtk(player);
        const baseStat = isMagic ? player.mag : effectiveAtk;
        const dealDamage = (base) => {
            const effected = this.applyOutgoingDamageEffects(player, base, {
                usesMagic: isMagic,
                targetBoss: true,
            });
            const isCritForced = Boolean(skill.isCrit);
        ...
        if (effect === "DAMAGE") {
            ...
            this.emitBattleEvent({ type: "BANNER", sourceId: player.id, targetId: (_d = this.boss) === null || _d === void 0 ? void 0 : _d.id, label: "Slow — boss พลาดโจมตี 35%", tone: "warning", fxPreset: "ice" });
        }
        else if (effect === "STUN") {
            dealDamage(Math.floor(baseStat * mult));
            if (Math.random() < 0.5) {
                this.applyStatusEffect(this.boss, { type: "STUN", turnsRemaining: 1, sourceId: player.id });
                this.emitBattleEvent({ type: "BANNER", sourceId: player.id, targetId: (_e = this.boss) === null || _e === void 0 ? void 0 : _e.id, label: "STUN! Boss ข้ามโจมตีถัดไป", tone: "warning", fxPreset: "thunder" });
            }
        }
            ...
            this.emitBattleEvent({ type: "BANNER", sourceId: player.id, targetId: (_f = this.boss) === null || _f === void 0 ? void 0 : _f.id, label: "Armor Pierced — boss รับดาเมจ +20%", tone: "skill", fxPreset: "pierce" });
        }
            ...
            this.emitBattleEvent({ type: "BANNER", sourceId: player.id, targetId: (_g = this.boss) === null || _g === void 0 ? void 0 : _g.id, label: `Poison ${poisonVal}/tick × ${poisonTurns}`, tone: "warning", fxPreset: "poison" });
        }
        else if (effect === "DEBUFF_ATK") {
            dealDamage(Math.floor(baseStat * mult));
            this.applyStatusEffect(this.boss, { type: "DEBUFF_ATK", turnsRemaining: 2, sourceId: player.id });
            this.emitBattleEvent({ type: "BANNER", sourceId: player.id, targetId: (_h = this.boss) === null || _h === void 0 ? void 0 : _h.id, label: "Boss ATK -30% 2 turns", tone: "skill", fxPreset: "debuff" });
        }
            ...
            this.emitBattleEvent({ type: "BANNER", sourceId: player.id, targetId: (_j = this.boss) === null || _j === void 0 ? void 0 : _j.id, label: "DEF BREAK — boss รับดาเมจ +50%", tone: "warning", fxPreset: "pierce" });
        }
            ...
            if (hpRatio < 0.30) {
                this.emitBattleEvent({ type: "BANNER", sourceId: player.id, targetId: (_k = this.boss) === null || _k === void 0 ? void 0 : _k.id, label: "EXECUTE! ×1.8 damage", tone: "danger", fxPreset: "execute" });
            }
            ...
            const healAmt = Math.floor(player.mag * ((_l = skill.healMultiplier) !== null && _l !== void 0 ? _l : 1.5));
            player.hp = Math.min(player.maxHp, player.hp + healAmt);
    ...
    handleBossDefeated() {
        ...
        const rewards = this.players.map((p) => {
            var _a, _b;
            const gold = Math.floor(200 * (1 + ((_a = p.goldMultiplier) !== null && _a !== void 0 ? _a : 0)));
            const xp = Math.floor(50 * (1 + ((_b = p.xpMultiplier) !== null && _b !== void 0 ? _b : 0)));
            p.earnedGold += gold;
            p.earnedXp += xp;
    ...
    handleMonsterDefeated(player) {
        ...
        const gold = Math.floor(rollGold(wave) * (1 + ((_a = player.goldMultiplier) !== null && _a !== void 0 ? _a : 0)));
        const xp = Math.floor(rollXp(wave) * (1 + ((_b = player.xpMultiplier) !== null && _b !== void 0 ? _b : 0)));
    ...
    executeSkillOnMonster(player, skillId, socket) {
        ...
        const isMagic = skill.damageBase === "MAG";
        const mult = (_b = skill.damageMultiplier) !== null && _b !== void 0 ? _b : (1 + skill.cost / 20);
        const effectiveAtk = this.getEffectivePlayerAtk(player);
        const baseStat = isMagic ? player.mag : effectiveAtk;
        const effect = skill.effect;
        const dealMonsterDamage = (base) => {
        ...
        if (effect === "DAMAGE" || effect === "LIFESTEAL") {
            ...
            const healAmt = Math.floor(player.mag * ((_c = skill.healMultiplier) !== null && _c !== void 0 ? _c : 1.5));
            player.hp = Math.min(player.maxHp, player.hp + healAmt);
            this.emitBattleEvent({ type: "HEAL_APPLIED", sourceId: player.id, targetId: player.id, amount: healAmt });
        }
        else {
