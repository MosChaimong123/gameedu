...
import type { GameSettings, NegamonBattlePlayer, NegamonBattleTuning } from "../types/game";
...
export class NegamonBattleEngine extends AbstractGameEngine {
    ...
    private resolveRound(): void {
        ...
        if (!q) {
            ...
            this.io.to(this.pin).emit("negamon-round-result", { logs: ["ข้ามรอบ — ไม่มีคำถาม"], players: this.players });
            this.broadcastState();
            return;
        }

        /** สแนปชันตอนปิดรอบ — ดาเมจรอบเดียวกันรวมก่อนลง HP ครั้งเดียว (simultaneous) */
        const active = [...this.activeCombatants()];
        const damageByTargetName = new Map<string, number>();
        ...
        for (const attacker of active) {
            ...
            const isFast = ans !== undefined && ans.at - this.roundStartedAt <= fastMs;
            for (const target of active) {
                ...
                const normalizedDamage = normalizeRoundDamage(damage, active.length);
                const prev = damageByTargetName.get(target.name) ?? 0;
                damageByTargetName.set(target.name, prev + normalizedDamage);
                logs.push(`${attacker.name} โจมตี ${target.name} -${normalizedDamage} HP`);
            }
        }

        for (const target of active) {
            const total = damageByTargetName.get(target.name) ?? 0;
            if (total <= 0) continue;
            target.battleHp = Math.max(0, target.battleHp - total);
            if (target.battleHp <= 0) {
                target.eliminated = true;
                target.eliminatedAt = Date.now();
                logs.push(`${target.name} K.O. (รวม -${total} HP ในรอบนี้)`);
            }
        }
        ...
        if (survivors.length <= 1) {
            this.io.to(this.pin).emit("negamon-round-result", { logs, players: this.players });
            this.broadcastState();
            this.endGame();
            return;
        }
