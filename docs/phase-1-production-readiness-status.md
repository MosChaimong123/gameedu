...
Last updated: 2026-05-17 (Stripe-only billing; Omise removed from app)
...
- **นอก repo** = Dashboard (Render, Atlas, Stripe, Sentry), DNS, ค่าทนาย, การรันคำสั่งต่อ **DB จริง** / ทดสอบมือบน URL จริง — ปิดงานด้วยการยืนยันใน **Completed production setup (log)** และ/หรือย้ายแถวในสรุปด้านล่าง
...
| Billing ในโค้ด | Stripe card + Stripe PromptPay; dev mock (`BILLING_THAI_PROVIDER=mock`); idempotency, `apply-plus-entitlement` path |
...
| ชำระเงิน | **Done:** Stripe test + **Stripe live smoke (card, 290 THB, 2026-05-07)**. **In repo (2026-05-17):** Omise removed; production uses Stripe only. **Open:** Stripe PromptPay live smoke — enable PromptPay + `checkout.session.async_payment_succeeded` on webhook; remove legacy `OMISE_*` / `BILLING_THAI_PROVIDER=omise` from Render. See [`phase-1-payment-readiness.md`](./phase-1-payment-readiness.md). |
...
| 2026-05-17 (in repo) | **Stripe-only billing:** Omise removed from upgrade UI, API routes, webhooks, and env schema. Production PLUS uses Stripe card subscription + Stripe PromptPay QR (`checkout.session.async_payment_succeeded`). Dev mock remains via `BILLING_THAI_PROVIDER=mock`. Ops: [`phase-1-payment-readiness.md`](./phase-1-payment-readiness.md) Render checklist + PromptPay smoke. |
...
Phase 1 is code-ready for a production beta foundation. Hosting and primary domain are in place; **core production smoke (auth, classroom, student code, live game + Socket)** is confirmed. **Stripe live card payment (290 THB)** verified (2026-05-07). **Public Terms + Privacy (EN/TH)** shipped in repo (2026-05-07). **Billing (2026-05-17):** Omise removed; Stripe PromptPay pass in repo — production still needs PromptPay enabled + live QR smoke. Remaining risk is mostly **data durability (backup)** (Phase 2 drill) and **extended smoke** (Negamon rewards, economy) when those paths matter for the pilot. **Monitoring** (Sentry + UptimeRobot) is live (2026-05-07). Optional: **institutional** DPA/MOU with counsel.
...
| Billing skeleton | Done in repo | Stripe card + PromptPay in repo; live card smoke 2026-05-07; PromptPay live smoke open |
...
| Payment provider verification | Partial | Stripe test + **Stripe live card smoke 290 THB** (2026-05-07). Omise retired (2026-05-17). Open: Stripe PromptPay live smoke + `async_payment_succeeded` on webhook. |
...
- Production billing is **Stripe only** (card + PromptPay QR). Do not reintroduce Omise env or UI. Stripe PromptPay live smoke is not logged yet (see payment readiness doc).
