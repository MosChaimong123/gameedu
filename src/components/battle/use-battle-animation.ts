...
import { resolveSkillVisual, type SkillFxPreset } from "@/components/battle/battle-animation-map";
import type { BattleAnimationEvent } from "@/lib/game/battle-events";

export type BattlePose = "idle" | "attack" | "cast" | "defend" | "hit" | "defeated";

export type FloatingPopup = {
  id: string;
  text: string;
  tone: "damage" | "heal" | "resource" | "crit" | "status";
  anchor: "player" | "enemy" | "center";
};

export type PersistentStatus = {
  id: string;
  kind: "poison" | "stun" | "shield" | "regen" | "buff" | "debuff" | "frost";
  label: string;
};
...
type BattleAnimationState = {
  ...
  feedbackSplash: { label: string; tone: "success" | "danger" } | null;
  popups: FloatingPopup[];
  activeSkillLabel: string | null;
  playerStatuses: PersistentStatus[];
  enemyStatuses: PersistentStatus[];
  activeFx: {
    id: string;
    label: string;
...
type UseBattleAnimationOptions = {
    ...
    newEvents.forEach((event) => {
      switch (event.type) {
        ...
        case "ACTION_DEFEND": {
          const isEnemySource = event.sourceRole === "enemy" || (event.sourceId != null && event.sourceId !== playerId);
          queueStatus(
            isEnemySource ? "enemy" : "player",
            { id: `${event.id}-shield`, kind: "shield", label: "Guard" },
            2600
          );
          setState((current) => ({
            ...current,
        ...
        case "ACTION_SKILL_CAST": {
          ...
          const visual = event.fxPreset && event.colorClass
            ...
            : resolveSkillVisual(event.skillId);
          ...
          const statusForPreset = (() => {
            switch (visual.effect ?? visual.preset) {
              case "POISON":
              case "poison":
                return { kind: "poison" as const, label: "Poison" };
              case "STUN":
              case "thunder":
                return { kind: "stun" as const, label: "Stun" };
              case "SLOW":
              case "ice":
                return { kind: "frost" as const, label: "Slow" };
              case "BUFF_DEF":
              case "DEFEND":
              case "shield":
                return { kind: "shield" as const, label: "Shield" };
              case "HEAL":
              case "REGEN":
              case "heal":
                return { kind: "regen" as const, label: "Regen" };
              case "BUFF_ATK":
              case "CRIT_BUFF":
              case "buff":
                return { kind: "buff" as const, label: "Buff" };
              case "DEBUFF_ATK":
              case "DEBUFF":
              case "debuff":
              case "DEF_BREAK":
              ...
              case "EXECUTE":
              case "execute":
                return { kind: "debuff" as const, label: "Break" };
              default:
                return null;
            }
          })();
          ...
          const target =
            event.targetId === playerId ? "player" : event.targetId === enemyId ? "enemy" : "center";
        ...
        case "RESOURCE_GAINED": {
          ...
          setState((current) => ({
            ...
            popups: [
              ...
              {
                id: event.id,
                text: `${prefix}${event.amount ?? 0}${suffix}`,
